...
import{y as w,_ as g,x as _,z as S}from"./index-Bs2UOTia.js";function M(a){return a.replace(new RegExp(" · ","g"),", ")}const u=a=>document.getElementById(a),y=a=>String(Math.round(a)).replace(/\B(?=(\d{3})+(?!\d))/g,".");let h="7",$=!1;async function T(){const a=u("andamentoBox");if(!a||$)return;const n=_(),e=!!n&&n.plan!=="free";if(!S())return;if(!e){a.innerHTML=`<div class="attesa">
  ...
  </div>`;const f=u("riallSi");f&&(f.onclick=async()=>{l.startDate=c;const{saveAll:v}=await g(async()=>{const{saveAll:m}=await import("./index-Bs2UOTia.js").then(b=>b.V);return{saveAll:m}},__vite__mapDeps([0,1]),import.meta.url);v();const{recomputeNow:p}=await g(async()=>{const{recomputeNow:m}=await import("./index-Bs2UOTia.js").then(b=>b.S);return{recomputeNow:m}},__vite__mapDeps([0,1]),import.meta.url);await p(),a.innerHTML=`<div class="riall fatto"><b>Ciclo riallineato al ${Number(i)}/${Number(s)}.</b></div>`});const N=u("riallNo");N&&(N.onclick=()=>{a.innerHTML=""})}export{T as paintAndamento,x as paintRiallineamento};
