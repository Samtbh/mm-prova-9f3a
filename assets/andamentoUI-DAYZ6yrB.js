...
import{y as w,_ as g,x as _,z as S}from"./index-CyBvDjXu.js";const u=a=>document.getElementById(a),y=a=>String(Math.round(a)).replace(/\B(?=(\d{3})+(?!\d))/g,".");let h="7",$=!1;async function M(){const a=u("andamentoBox");if(!a||$)return;const n=_(),e=!!n&&n.plan!=="free";if(!S())return;if(!e){a.innerHTML=`<div class="attesa">
     ...
     <p class="and-say">Su <b>${c} giornate chiuse</b> hai mangiato in media
       ...
       che ti avevo dato${t.balanceTag?`. ${String(t.balanceTag)}`:""}.</p>`+(d.length?`<h3>Cosa vedo</h3><div class="grp">${d.map(i=>`<div class="g-row"><div class="t"><b>${i.title}</b><span>${i.body}</span></div>
            <span class="val" style="color:${i.severity==="warn"?"var(--warn)":"var(--muted-2)"}">${i.severity==="warn"?"da guardare":"nota"}</span></div>`).join("")}</div>`:'<p class="and-say" style="color:var(--pos)">Nessuna abitudine storta in questo periodo.</p>'),a.querySelectorAll("#andPeriodo .splitchip").forEach(i=>{i.onclick=()=>{h=i.dataset.p,M()}})}async function L(){const a=u("riallineaBox");if(!a)return;a.innerHTML="";const n=await w("ciclo",{});if(!n)return;const e=n.slittamento??{},r=n.ancoraSuggerita??{},{cycle:l}=await g(async()=>{const{cycle:v}=await import("./index-CyBvDjXu.js").then(p=>p.T);return{cycle:v}},__vite__mapDeps([0,1]),import.meta.url),t=Math.abs(Number(e.giorni??0))>0,o=Number(r.confidence??0)>=.7,d=!!r.anchor&&r.anchor!==l.startDate;if(!t&&!(o&&d))return;const c=r.anchor??"",[,s,i]=c.split("-");a.innerHTML=`<div class="riall">
  ...
  </div>`;const f=u("riallSi");f&&(f.onclick=async()=>{l.startDate=c;const{saveAll:v}=await g(async()=>{const{saveAll:m}=await import("./index-CyBvDjXu.js").then(b=>b.V);return{saveAll:m}},__vite__mapDeps([0,1]),import.meta.url);v();const{recomputeNow:p}=await g(async()=>{const{recomputeNow:m}=await import("./index-CyBvDjXu.js").then(b=>b.S);return{recomputeNow:m}},__vite__mapDeps([0,1]),import.meta.url);await p(),a.innerHTML=`<div class="riall fatto"><b>Ciclo riallineato al ${Number(i)}/${Number(s)}.</b></div>`});const N=u("riallNo");N&&(N.onclick=()=>{a.innerHTML=""})}export{M as paintAndamento,L as paintRiallineamento};
