...
import{x as w,_ as g,w as L,y as _}from"./index-DltX2XKJ.js";const u=a=>document.getElementById(a),y=a=>String(Math.round(a)).replace(/\B(?=(\d{3})+(?!\d))/g,".");let h="7",$=!1;async function S(){const a=u("andamentoBox");if(!a||$)return;const e=L(),n=!!e&&e.plan!=="free";if(!_())return;if(!n){a.innerHTML=`<div class="attesa">
  ...
  </div>`;const f=u("riallSi");f&&(f.onclick=async()=>{l.startDate=c;const{saveAll:p}=await g(async()=>{const{saveAll:m}=await import("./index-DltX2XKJ.js").then(b=>b.N);return{saveAll:m}},__vite__mapDeps([0,1]),import.meta.url);p();const{recomputeNow:v}=await g(async()=>{const{recomputeNow:m}=await import("./index-DltX2XKJ.js").then(b=>b.K);return{recomputeNow:m}},__vite__mapDeps([0,1]),import.meta.url);await v(),a.innerHTML=`<div class="riall fatto"><b>Ciclo riallineato al ${Number(i)}/${Number(s)}.</b></div>`});const N=u("riallNo");N&&(N.onclick=()=>{a.innerHTML=""})}export{S as paintAndamento,M as paintRiallineamento};
