...
import{x as w,_ as g,w as L,y as _}from"./index-iIrgOqzX.js";const u=a=>document.getElementById(a),y=a=>String(Math.round(a)).replace(/\B(?=(\d{3})+(?!\d))/g,".");let h="7",$=!1;async function S(){const a=u("andamentoBox");if(!a||$)return;const e=L(),n=!!e&&e.plan!=="free";if(!_())return;if(!n){a.innerHTML=`<div class="attesa">
     ...
     <p class="and-say">Su <b>${c} giornate chiuse</b> hai mangiato in media
       ...
       che ti avevo dato${t.balanceTag?` — ${String(t.balanceTag).toLowerCase()}`:""}.</p>`+(d.length?`<h3>Cosa vedo</h3><div class="grp">${d.map(i=>`<div class="g-row"><div class="t"><b>${i.title}</b><span>${i.body}</span></div>
            <span class="val" style="color:${i.severity==="warn"?"var(--warn)":"var(--muted-2)"}">${i.severity==="warn"?"da guardare":"nota"}</span></div>`).join("")}</div>`:'<p class="and-say" style="color:var(--pos)">Nessuna abitudine storta in questo periodo.</p>'),a.querySelectorAll("#andPeriodo .splitchip").forEach(i=>{i.onclick=()=>{h=i.dataset.p,S()}})}async function T(){const a=u("riallineaBox");if(!a)return;a.innerHTML="";const e=await w("ciclo",{});if(!e)return;const n=e.slittamento??{},r=e.ancoraSuggerita??{},{cycle:l}=await g(async()=>{const{cycle:p}=await import("./index-iIrgOqzX.js").then(v=>v.M);return{cycle:p}},__vite__mapDeps([0,1]),import.meta.url),t=Math.abs(Number(n.giorni??0))>0,o=Number(r.confidence??0)>=.7,d=!!r.anchor&&r.anchor!==l.startDate;if(!t&&!(o&&d))return;const c=r.anchor??"",[,s,i]=c.split("-");a.innerHTML=`<div class="riall">
  ...
  </div>`;const f=u("riallSi");f&&(f.onclick=async()=>{l.startDate=c;const{saveAll:p}=await g(async()=>{const{saveAll:m}=await import("./index-iIrgOqzX.js").then(b=>b.O);return{saveAll:m}},__vite__mapDeps([0,1]),import.meta.url);p();const{recomputeNow:v}=await g(async()=>{const{recomputeNow:m}=await import("./index-iIrgOqzX.js").then(b=>b.L);return{recomputeNow:m}},__vite__mapDeps([0,1]),import.meta.url);await v(),a.innerHTML=`<div class="riall fatto"><b>Ciclo riallineato al ${Number(i)}/${Number(s)}.</b></div>`});const N=u("riallNo");N&&(N.onclick=()=>{a.innerHTML=""})}export{S as paintAndamento,T as paintRiallineamento};
