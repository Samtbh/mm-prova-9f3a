import{w as g}from"./index-CjoNceTr.js";import{paintPremi as S,premi as $}from"./premiUI-Bglm2pT6.js";const a=t=>document.getElementById(t),l=t=>String(Math.round(t)).replace(/\B(?=(\d{3})+(?!\d))/g,"."),q=t=>t===1?"1 giorno":`${t} giorni`;function C(t){return Math.abs(t)<=100?{parola:"In linea",classe:"ok"}:t>0?{parola:"Sopra",classe:"att"}:{parola:"Sotto",classe:"att"}}function M(t,n){return n&&t>1?"È il tuo record.":t===30?"Un mese intero.":t===14?"Hai costruito la costanza.":t===7?"Non è più un caso.":t===3?"Tre di fila.":""}async function w(t){const n=a("qSheet"),d=a("qBody"),i=a("qSave"),o=a("qCancel");if(!n||!d||!i)return;await S().catch(()=>{});const e=$(),c=e?.striscia??0,v=!!e&&e.striscia>=e.strisciaMigliore&&e.striscia>0,u=C(t.difference),m=M(c,v),s=g()?.plan,b=!s||s==="free"||s==="base",r=a("qEt");r&&(r.textContent="Giornata chiusa",r.hidden=!1);const f=a("qScala");f&&(f.hidden=!0),a("qTitle").textContent=c?`${q(c)} di fila`:"Giornata chiusa",a("qSub").textContent=m,d.innerHTML=`
    <div class="ch-conto">
    ...
    ${c>=14&&b?`<div class="ch-oltre">
           <p>Hai costruito la costanza. Ora usala.</p>
