const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["./qualifica-NU4IgO-9.js","./index-CR9W2L_a.js","./index-BzX70f7P.css"])))=>i.map(i=>d[i]);
import{v as m,_ as N,w as G}from"./index-CR9W2L_a.js";const e=o=>document.getElementById(o);let g=[],f=[];const H=()=>!!G()?.professional;async function j(){if(!H())return;const[o,i]=await Promise.all([m.from("mm2_protocols").select("*").order("created_at"),m.from("mm2_clients").select("*").order("created_at")]);g=o.data??[],f=i.data??[];const t=await N(()=>import("./qualifica-NU4IgO-9.js"),__vite__mapDeps([0,1,2]),import.meta.url);L=await t.puoNutrizione(),await t.qualifica()||t.chiediQualifica();const{segnaClienti:a}=await N(async()=>{const{segnaClienti:r}=await import("./index-CR9W2L_a.js").then(c=>c.T);return{segnaClienti:r}},__vite__mapDeps([1,2]),import.meta.url);a(f.length)}const F=o=>o.goal_mode==="maintenance"?"Mantenimento":`${o.goal_mode==="deficit"?"Deficit":"Surplus"} ${o.goal_pct}%`;let L=!1;const K=o=>L?`${F(o)} · ciclo ${o.cycle_days} giorni · ${o.sessions} allenamenti · ${o.protein_g_kg} g/kg proteine`:`ciclo ${o.cycle_days} giorni · ${o.sessions} allenamenti`,C="Riposo",Q=[2500,3200],U=`<b>Perché i carboidrati non si scrivono.</b> Il numero del giorno di ogni persona
   ...
   Qui sotto vedi quanti ne restano, in grammi e come rapporto sui grassi.`;function z(o,i){if(o==="maintenance"||!i)return"Nessuno scostamento: il numero resta quello che consuma.";const t=o==="deficit"?"−":"+",a=c=>String(Math.round(c)).replace(/\B(?=(\d{3})+(?!\d))/g,"."),r=Q.map(c=>`${t}${a(c*i/100)} kcal su ${a(c)}`).join(" · ");return`${i}% vuol dire ${r}.`}const V=75;function Z(o,i,t){const a=V,r=i*a,c=t*a,p=(o-(r*4+c*9))/4;if(p<=0)return`⚠️ Su ${a} kg, proteine e grassi da soli fanno ${Math.round(r*4+c*9)} kcal: più del numero (${Math.round(o)}). Non resterebbe niente per i carboidrati. Abbassa i g/kg.`;const v=(p/c).toFixed(1).replace(".",",");return`Su ${a} kg e ${(d=>String(Math.round(d)).replace(/\B(?=(\d{3})+(?!\d))/g,"."))(o)} kcal: ${Math.round(r)} g proteine · ${Math.round(c)} g grassi · ${Math.round(p)} g carboidrati (${(p/a).toFixed(1).replace(".",",")} g/kg). <b>C:G ${v} : 1</b>.`}const J=`<b>Percentuale di cosa.</b> Di quello che la persona consuma in un giorno
   ...
   stai ancora cambiando il numero di nessuno.`;function R(o,i){const t=Math.max(2,Math.min(15,Math.round(i??o?.cycle_days??7))),a=Array.isArray(o?.split)?o.split:[];return Array.from({length:t},(r,c)=>({label:String(a[c]?.label??C),trainingKcal:Number(a[c]?.trainingKcal??0)}))}const W=o=>{const i=Array.isArray(o.split)?o.split:[];return i.length?i.map(t=>t.label||C).join(" · "):""};function X(){const o=e("protocolliBox");if(o){if(!g.length){o.innerHTML=`<div class="vuoto-ok" style="margin-top:16px">
    ...
    </div>`;const i=e("vuotoProto");i&&(i.onclick=()=>{E()});return}o.innerHTML='<div class="grp" style="margin-top:16px">'+g.map(i=>{const t=f.filter(r=>r.protocol_id===i.id).length,a=W(i);return`<div class="g-row"><div class="t"><b>${i.name}</b><span>${K(i)}${a?`<br><i class="ps-ciclo">${a}</i>`:""}</span></div>
        <span class="val">${t?`${t} client${t===1?"e":"i"}`:"nessun cliente"}
        <button class="mini" data-mod="${i.id}">modifica</button></span></div>`}).join("")+"</div>",o.querySelectorAll("[data-mod]").forEach(i=>{i.onclick=()=>{E(g.find(t=>t.id===i.dataset.mod))}})}}async function E(o){const{puoNutrizione:i}=await N(async()=>{const{puoNutrizione:n}=await import("./qualifica-NU4IgO-9.js");return{puoNutrizione:n}},__vite__mapDeps([0,1,2]),import.meta.url),t=await i(),a=e("qSheet"),r=e("qBody"),c=e("qTitle"),_=e("qSub"),p=e("qSave"),v=e("qCancel");if(!a||!r||!p)return;c.textContent=o?"Modifica protocollo":"Nuovo protocollo",_.textContent="Quello che scrivi qui diventa il piano di chi lo riceve.";const l=(n,s)=>String(o?.[n]??s);r.innerHTML=`<label class="mn-l" for="pName">Nome</label>
    ...
    </div>`;const i=e("vuotoCli");i&&(i.onclick=()=>I());return}o.innerHTML='<div class="grp" style="margin-top:16px">'+f.map(i=>{const t=g.find(r=>r.id===i.protocol_id),a=i.linked_user?'<span class="val">collegato</span>':'<span class="val" style="color:var(--muted-2)">non collegato</span>';return`<div class="g-row" data-cli="${i.id}" style="cursor:pointer">
     ...
     <div class="splitchips" id="cProt">
       ...
       ${g.map(l=>`<button type="button" class="splitchip ${o?.protocol_id===l.id?"on":""}" data-p="${l.id}">${l.name}</button>`).join("")}
     </div>
